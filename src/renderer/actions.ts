...
export class Dispatcher {
  ...
  constructor() {
    ...
    this.stories = {
      ...
      updateDesc: (uuid: string, desc: string) =>
        this._dispatch({ type: 'STORIES_UPDATE_DESC', payload: { uuid, desc } }),
      updateMemo: (uuid: string, memo: string) =>
        this._dispatch({ type: 'STORIES_UPDATE_MEMO', payload: { uuid, memo } }),
      changeTopicId: (uuid: string, topicId: string) =>
        this._dispatch({ type: 'STORIES_CHANGE_TOPIC_ID', payload: { uuid, topicId } })
    }
    this.story = {
